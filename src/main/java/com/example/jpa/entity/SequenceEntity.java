...
public class SequenceEntity {
     ...
     *      이때 메모리에 순차적으로 증가한 값을 보관 --> 이 과정 때문에 allocationSize를 1로 설정하지 않았을 경우, 생여러 JVM이 가동되는 가정 하에 충돌이 발생하게 됨
    ...
    initialValue : DDL을 통해 데이터베이스를 생성할 때 사용되는 값으로, 최초 설정되는 값을 정의 (default : 0)
